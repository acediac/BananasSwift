...
class AAPLGameSimulation: SCNScene, SCNSceneRendererDelegate {
    ...
    */
    ...
    func setupTechniques () {
        ...
        var url: NSURL = NSBundle.mainBundle().URLForResource("art.scnassets/techniques/desaturation", withExtension: "plist")!
        
        var contentsOfUrlAsDictionary = NSDictionary(contentsOfURL: url)
        
        self.desaturationTechnique = SCNTechnique(dictionary: NSDictionary(contentsOfURL: url)! as Dictionary<NSObject, AnyObject>)!
...
// MARK: Resource Loading convenience
    ...
    static func loadParticleSystemWithName(name: String) -> SCNParticleSystem? {
        ...
        if var newSystem = NSKeyedUnarchiver.unarchiveObjectWithFile(path) as? SCNParticleSystem {
            if let _particleImage = newSystem.particleImage as? NSURL,
                _particleImagePath = _particleImage.path {
                path = String(format: "level/effects/%@", _particleImagePath.lastPathComponent)
                path = self.pathForArtResource(path)
                let url = NSURL(string: path.stringByAddingPercentEscapesUsingEncoding(NSUnicodeStringEncoding)!)
