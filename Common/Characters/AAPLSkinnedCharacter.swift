...
class AAPLSkinnedCharacter: SCNNode {
    ...
    class func loadAnimationNamed(animationName:String, sceneName:String) -> CAAnimation? {
        ...
        var animation: CAAnimation? = sceneSource?.entryWithIdentifier(animationName, withClass: CAAnimation.self) as? CAAnimation
    ...
    func chainAnimation(firstKey:String, secondKey:String, fadeTime:CGFloat) {
        let firstAnim: CAAnimation? = self.cachedAnimation(firstKey)
        let secondAnim: CAAnimation? = self.cachedAnimation(secondKey)
